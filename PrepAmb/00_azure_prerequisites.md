...
Recursos de Azure necesarios para la capa Raw, provisionados una sola vez fuera del
pipeline. Los pasos 1–5 documentan cómo se crea cada uno (referencia para recrearlos en
otra suscripción/región).
...
| External Location (Unity Catalog) | `raw_ext_loc` — sobre `abfss://raw@stdbkprojectsraw.dfs.core.windows.net/`, con `GRANT READ FILES` a `account users` |
...
`ACCESS_CONNECTOR_ID` en [03_storage_credential.py](03_storage_credential.py).
...
`STORAGE_ACCOUNT`/`CONTAINER_NAME` quedan fijos dentro de
[04_external_location.sql](04_external_location.sql) (`stdbkprojectsraw`/`raw`).
